...
class ArticleListCreateView(generics.ListCreateAPIView):
    ...
    filterser_class = ArticleFilter
    ordering_fields = ["created_at", "updated_at",]
    renderer_classes = [ArticlesJSONRenderer]

    def perform_create(self, serializer):
        serializer.save(author=self.request.user)
        logger.info(f"article {serializer.data.get('title')} created by {self.
...
class ArticleRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        ArticleView.record_view(article=instance, user=request.user,
                                viewer_ip=viewer_ip)
...
class ClapArticleView(generics.CreateAPIView, generics.DestroyAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"detail": "You have already clapped!"},
